const EventService = {
  ...
  saveCategories(categories) {
    ...
    if (duplicateName) {
      throw new Error(
        `Category: "${duplicateName}" already exists. Pick a different name.`,
      );
    }

    let eventCategories = this.getEventCategories();
    const newCategoryIds = categories.map((category) => category.id);
    eventCategories = eventCategories.filter((conn) =>
  ...
  deleteEvent(eventId) {
    ...
    const eventCategories = this.getEventCategories().filter(
      (conn) => conn.eventId !== eventId,
    );
    this.saveEventCategories(eventCategories);
  },

  getEventsWithCategories(filter = {}) {
    const { selectedCategories = [], timeRange = { from: null, to: null } } =
      filter;
    ...
    return events
      ...
      .filter((event) => {
        if (selectedCategories.length > 0) {
          const eventCategoryIds = event.categories.map(
  ...
  findEventWithCategories(eventId) {
    return (
      this.getEventsWithCategories().find((evt) => evt.id === eventId) || null
    );
  },
  ...
  upsertEventWithCategories(eventWithCategories) {
    ...
    if (
      !eventWithCategories.id ||
      !events.some((e) => e.id === eventWithCategories.id)
    ) {
